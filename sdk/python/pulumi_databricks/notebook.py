# coding=utf-8
# *** WARNING: this file was generated by the Pulumi Terraform Bridge (tfgen) Tool. ***
# *** Do not edit by hand unless you're certain you know what you are doing! ***

import json
import warnings
import pulumi
import pulumi.runtime
from typing import Union
from . import utilities, tables

class Notebook(pulumi.CustomResource):
    content: pulumi.Output[str]
    format: pulumi.Output[str]
    language: pulumi.Output[str]
    mkdirs: pulumi.Output[bool]
    object_id: pulumi.Output[float]
    object_type: pulumi.Output[str]
    overwrite: pulumi.Output[bool]
    path: pulumi.Output[str]
    def __init__(__self__, resource_name, opts=None, content=None, format=None, language=None, mkdirs=None, overwrite=None, path=None, __props__=None, __name__=None, __opts__=None):
        """
        Create a Notebook resource with the given unique name, props, and options.
        :param str resource_name: The name of the resource.
        :param pulumi.ResourceOptions opts: Options for the resource.
        """
        if __name__ is not None:
            warnings.warn("explicit use of __name__ is deprecated", DeprecationWarning)
            resource_name = __name__
        if __opts__ is not None:
            warnings.warn("explicit use of __opts__ is deprecated, use 'opts' instead", DeprecationWarning)
            opts = __opts__
        if opts is None:
            opts = pulumi.ResourceOptions()
        if not isinstance(opts, pulumi.ResourceOptions):
            raise TypeError('Expected resource options to be a ResourceOptions instance')
        if opts.version is None:
            opts.version = utilities.get_version()
        if opts.id is None:
            if __props__ is not None:
                raise TypeError('__props__ is only valid when passed in combination with a valid opts.id to get an existing resource')
            __props__ = dict()

            if content is None:
                raise TypeError("Missing required property 'content'")
            __props__['content'] = content
            __props__['format'] = format
            __props__['language'] = language
            __props__['mkdirs'] = mkdirs
            __props__['overwrite'] = overwrite
            if path is None:
                raise TypeError("Missing required property 'path'")
            __props__['path'] = path
            __props__['object_id'] = None
            __props__['object_type'] = None
        super(Notebook, __self__).__init__(
            'databricks:index/notebook:Notebook',
            resource_name,
            __props__,
            opts)

    @staticmethod
    def get(resource_name, id, opts=None, content=None, format=None, language=None, mkdirs=None, object_id=None, object_type=None, overwrite=None, path=None):
        """
        Get an existing Notebook resource's state with the given name, id, and optional extra
        properties used to qualify the lookup.

        :param str resource_name: The unique name of the resulting resource.
        :param str id: The unique provider ID of the resource to lookup.
        :param pulumi.ResourceOptions opts: Options for the resource.
        """
        opts = pulumi.ResourceOptions.merge(opts, pulumi.ResourceOptions(id=id))

        __props__ = dict()

        __props__["content"] = content
        __props__["format"] = format
        __props__["language"] = language
        __props__["mkdirs"] = mkdirs
        __props__["object_id"] = object_id
        __props__["object_type"] = object_type
        __props__["overwrite"] = overwrite
        __props__["path"] = path
        return Notebook(resource_name, opts=opts, __props__=__props__)
    def translate_output_property(self, prop):
        return tables._CAMEL_TO_SNAKE_CASE_TABLE.get(prop) or prop

    def translate_input_property(self, prop):
        return tables._SNAKE_TO_CAMEL_CASE_TABLE.get(prop) or prop

