...
First, clone this repo with the name of the desired provider in place of `databricks`:

```
git clone https://github.com/pulumi/pulumi-tf-provider-boilerplate pulumi-databricks
```

Second, replace references to `databricks` with the name of your provider:
...
The following configuration points are available for the `databricks` provider:

- `databricks:apiKey` (environment: `XYZ_API_KEY`) - the API key for `databricks`
- `databricks:region` (environment: `XYZ_REGION`) - the region in which to deploy resources
